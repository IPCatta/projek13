...
// Cari file FPDF di beberapa lokasi yang umum
$fpdfFile = null;
...
} elseif (file_exists(__DIR__ . '/fpdf186fpdf.php')) {
    $fpdfFile = __DIR__ . '/fpdf186fpdf.php';
}
if (!$fpdfFile) {
    die("FPDF library not found. Please download it from https://www.fpdf.org/ and extract so that 'fpdf.php' is located at 'fpdf186/fpdf.php' or copy it to the project root as 'fpdf.php'.");
}
require_once $fpdfFile;
if (!class_exists('FPDF', false)) {
    die("FPDF class not found in included file ($fpdfFile). Please ensure you've installed the official FPDF library (https://www.fpdf.org/).");
}
require_once __DIR__ . '/koneksi.php'; // File untuk koneksi ke database
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $pdf->Cell(30, 10, $row['tanggalLahir'], 1, 1, 'C');
    $no++;
}
